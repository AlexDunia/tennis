...
test('all active Ladder entry callers target LiveMatch and Tournament has migrated too', () => {
  ...
  assert.match(hub, /PLAY_MATCH_ACTIONS/)
  assert.match(hub, /getPlayMatchActions/)
  assert.match(hub, /case PLAY_MATCH_ACTIONS\.START_MATCH:[\s\S]*name:\s*'LiveMatch'/)
  assert.match(hub, /case PLAY_MATCH_ACTIONS\.VIEW_MATCH:[\s\S]*name:\s*'MatchDetails'/)
  assert.match(hub, /case PLAY_MATCH_ACTIONS\.VIEW_LIVE_SCORE:[\s\S]*name:\s*'LiveScoreboard'/)
  assert.match(hub, /case PLAY_MATCH_ACTIONS\.OPEN_MATCH_CONTROL:[\s\S]*name:\s*'LiveOperationDetail'/)

  const viewMatchStart = hub.indexOf('case PLAY_MATCH_ACTIONS.VIEW_MATCH:')
  const viewMatchEnd = hub.indexOf('case PLAY_MATCH_ACTIONS.', viewMatchStart + 1)
  const viewMatchCase = hub.slice(viewMatchStart, viewMatchEnd)
  assert.doesNotMatch(viewMatchCase, /explicitStart/)
  assert.match(ladder, /async function viewMatch[\s\S]*name: 'MatchDetails'/)
  assert.match(details, /case PLAY_MATCH_ACTIONS\.START_MATCH:[\s\S]*name: 'LiveMatch'/)
  assert.doesNotMatch(details, /openLadderLive|canOpenLadderLive/)
