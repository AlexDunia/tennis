...
const isDirectoryExperience = computed(() => routeView.value.startsWith('directory'))
...
function flowLocation(query) {
  return router.resolve({ path: route.path, query })
}

function setQuery(query, { replace = false } = {}) {
  pageError.value = ''
  pageMessage.value = ''
  const location = flowLocation(query)
  return replace ? router.replace(location) : router.push(location)
}

function goToPreviousFlowState(query) {
  const previousLocation = flowLocation(query)
  if (window.history.state?.back === previousLocation.fullPath) {
    router.back()
    return
  }

  // A resumed or directly linked step may not have its logical predecessor in
  // browser history. Replace it with that predecessor instead of escaping the flow.
...
function openJoinClubFlow() {
  router.push({ name: 'PlayerClubJoin' })
}

useShellNestedHeader(() => {
  if (route.name !== 'Clubs') return null

  if (routeView.value === 'directory-create') {
    return {
      label: 'Create club',
      subtitle: 'Add the basics. You can set up everything else later.',
      backLabel: 'Back to Club',
      back: showClubDirectory,
      crumbs: [],
    }
  }

  if (routeView.value === 'directory-join') {
    return {
      label: 'Join a club',
      subtitle: 'Use the invitation sent by your club.',
      backLabel: 'Back to Club',
      back: showClubDirectory,
      crumbs: [],
    }
...
function goBack() {
  ...
  if (routeView.value === 'join') return goToPreviousFlowState({ view: 'start' })
  if (routeView.value === 'join-confirm') return goToPreviousFlowState({ view: 'join' })
  if (stepIndex.value === 0) {
    goToPreviousFlowState({ view: 'start' })
    return
  }
...
<template>
  <main
    ...
    class="gorra-club-ref ref-page ref-clubs-directory-page"
  ...
  >
    ...
    >
      <label class="answer-field">
        <span>Invitation code or link</span>
        <input
          v-model="inviteCode"
          maxlength="2048"
          autocomplete="one-time-code"
          placeholder="Paste the invitation sent by your club"
          required
      ...
      >
        ...
        <span>
          ...
          <small>Your relationship will be {{ pendingInvite.roleLabel }}.</small>
        </span>
        <FlowIcon name="check" />
      </section>
    ...
    <div v-else class="club-directory-hub">
      <section class="club-directory-section" aria-label="Club actions">
        <div class="club-entry-options">
          ...
          >
            <span class="club-entry-icon" aria-hidden="true">
              <FlowIcon name="login" />
            </span>

            <span class="club-entry-copy">
      ...
      >
        <header class="club-section-heading">
          <div class="club-section-heading__row">
            ...
            <span class="club-section-heading__count">
              ...
              {{ directoryClubs.length === 1 ? 'club' : 'clubs' }}
            </span>
          </div>

          <p>Open a club you belong to.</p>
        </header>
        ...
        <EmptyState
          ...
          illustration="home"
          title="No clubs yet"
          description="Clubs you join or create will appear here."
        />
      </section>
    </div>
  </main>
...
.clubs-directory__header > p:last-child {
  max-width: 58ch;
  margin: 0;
  color: #647067;
  font-size: 14px;
...
.join-preview {
  ...
  gap: 14px;
  padding: 15px;
  border: 1px solid rgba(40, 122, 69, 0.24);
  border-radius: 14px;
  background: #f4faf5;
}
.join-preview > span:nth-child(2) {
  display: grid;
  gap: 4px;
}
.join-preview small {
...
.relationship-submit {
  --club-control-height: 44px;
  --club-control-padding: 18px;
  min-width: 132px;
  min-height: 44px;
  padding-inline: 18px;
  justify-self: start;
