...
function matchLinkForPlayer(playerId) {
  ...
  if (draft) {
    ...
    return {
      kind: 'draft',
      draft,
      summary: `Linked with ${teammate.name} \\u00B7 needs a date & time`,
    }
  }
  ...
  return {
    ...
    summary: challenge.scheduledAt
      ? `Linked with ${teammate.name} \\u00B7 scheduled`
      : `Linked with ${teammate.name} \\u00B7 needs a date & time`,
  }
}
...
/* Mirrors the Individual mode header; only the workspace changes when mode changes. */
.bulk-players__setup { display: grid; justify-items: end; gap: 5px; margin-left: auto; }
.bulk-players__setup > span { color: var(--color-muted); font-size: 9px; font-weight: var(--font-weight-semibold); letter-spacing: .07em; text-transform: uppercase; }
.bulk-mode-tabs { display: inline-grid; grid-template-columns: 1fr 1fr; gap: 3px; padding: 3px; border-radius: 8px; background: var(--color-surface-soft); }
.bulk-mode-tabs button { min-width: 82px; min-height: 32px; padding: 0 11px; border: 0; border-radius: 6px; background: transparent; color: var(--color-muted); font-size: 10px; font-weight: var(--font-weight-semibold); cursor: pointer; }
.bulk-mode-tabs button.active { background: #111; color: #fff; }
.bulk-mode-tabs button:focus-visible { outline: 2px solid var(--color-primary-strong); outline-offset: 2px; }
/* Keep Bulk on the same persistent visual layer as Individual. */
.bulk-players__head { z-index: 30; }
/* Bulk uses the same primary header hierarchy as Individual. */
.bulk-players__head { align-items: center; }
.bulk-players__head h1 { order: 2; color: var(--color-muted); font-size: 12px; font-weight: var(--font-weight-semibold); letter-spacing: 0; }
.bulk-players__setup { order: 1; justify-items: start; margin-left: 0; }
.bulk-players__setup > span { display: none; }
.bulk-players__head .bulk-mode-tabs { gap: 0; padding: 0; border-radius: 0; background: transparent; }
.bulk-players__head .bulk-mode-tabs button { min-width: 124px; min-height: 44px; padding: 0 18px; border-radius: 0; color: var(--color-muted); font-size: 15px; font-weight: var(--font-weight-bold); }
.bulk-players__head .bulk-mode-tabs button.active { border-bottom: 3px solid var(--color-primary-strong); background: transparent; box-shadow: none; color: var(--color-text); }
/* Matches the Individual header exactly. */
.bulk-players__head h1 { order: 0; color: var(--color-text); font-size: 23px; font-weight: var(--font-weight-bold); letter-spacing: -0.025em; }
.bulk-players__setup { order: 0; justify-items: end; margin-left: auto; }
.bulk-players__setup > span { display: none; }
.bulk-players__head .bulk-mode-tabs { gap: 3px; padding: 3px; border-radius: 8px; background: var(--color-surface-soft); }
.bulk-players__head .bulk-mode-tabs button { min-width: 82px; min-height: 34px; padding: 0 11px; border-radius: 6px; font-size: 10px; }
.bulk-players__head .bulk-mode-tabs button.active { border-bottom: 0; background: #111; box-shadow: 0 2px 8px rgba(0, 0, 0, .16); color: #fff; }
/* Use the exact Individual header track: changing mode cannot move the toggle. */
.bulk-players__head {
  justify-content: space-between;
  margin: 0 -30px 14px;
  padding: 24px 30px 14px;
