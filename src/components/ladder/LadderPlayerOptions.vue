...
<template>
  <section
    class="ladder-player-options"
    @click.stop
  >
    <div class="ladder-player-options__line ladder-player-options__line--primary">
      <button
        type="button"
        :disabled="position <= 1"
        @click="emit('move-up')"
      ...
      >
        <span
          class="ladder-player-options__hash"
          aria-hidden="true"
        >
          #
        </span>
    ...
    <div class="ladder-player-options__line ladder-player-options__line--secondary">
      <button
        type="button"
...
.ladder-player-options {
  ...
  border: 1px solid
    var(--color-border, #e4e9e5);
  border-top-color:
    rgba(44, 58, 48, 0.07);
  border-radius:
    0 0
    var(--app-card-radius, 12px)
    var(--app-card-radius, 12px);
  background: #fbfcfb;
}

.ladder-player-options__line {
  display: flex;
  min-width: 0;
  align-items: center;
  gap: 8px;
...
/*
 * Desktop is intentionally TWO visible lines.
 * No horizontal scrolling.
 *
 * Line one ends after Pause challenges.
 * Line two contains the remaining actions.
 */
...
.ladder-player-options__remove {
  border-color:
    rgba(154, 81, 76, 0.16) !important;
  background:
    rgba(154, 81, 76, 0.035) !important;
  color: #9a514c !important;
}

.ladder-player-options__remove:hover {
  border-color:
    rgba(154, 81, 76, 0.28) !important;
  background:
    rgba(154, 81, 76, 0.065) !important;
}
